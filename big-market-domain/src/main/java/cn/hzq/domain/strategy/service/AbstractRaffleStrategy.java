...
public abstract class AbstractRaffleStrategy implements IRaffleStrategy {
    ...
    // 抽奖的责任链 -> 从抽奖规则中，解耦出前置规则为责任链处理
    protected DefaultChainFactory defaultChainFactory;
    // 抽奖的规则树 -> 负责抽奖中到抽奖后的规则过滤。如抽到A奖品ID，之后要做次数判断和库存扣减
    ...
    public RaffleAwardEntity performRaffle(RaffleFactorEntity raffleFactorEntity) {
        ...
        // 2. 责任链抽奖计算【这一步拿到最初的奖品ID,之后根据ID处理抽奖】 注意：黑名单、权重等非默认抽奖直接返回抽奖结果
        ...
        DefaultTreeFactory.StrategyAwardVO treeStrategyAwardVO =
                raffleLogicTree(userId, strategyId, chainStrategyAwardVO.getAwardId(), raffleFactorEntity.getEndDateTime());
